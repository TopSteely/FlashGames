...
class Q_Learning_Control(object):
    ...
    def update_policy_online(self, state, state_new, action, reward):
        if self.in_use:
            ...
            print 'after'
            pdb.set_trace()
            sa_n = self.state_action_count.get_value(state, action)# = None! Why?????
            print 'sa:', sa_n, 'q_old', q_old
            alternative_action = self.Q.get_action(state_new, True)
            alternative_q = self.Q.get_value(state_new, [alternative_action])
            print (reward + alternative_q - q_old), sa_n, (reward + alternative_q - q_old) / sa_n
